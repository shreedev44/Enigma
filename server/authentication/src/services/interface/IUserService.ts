import { GoogleAuthUserType, LoginResponseType, Role, UserType } from '@types'

export interface IUserService {
    register(user: UserType): Promise<string>
    verifyOtp(otp: string, email: string): Promise<void>
    resendOtp(email: string): Promise<void>
    verifyUser(email: string, password: string, role: Role): Promise<LoginResponseType>
    ...
    getRecruiter(recruiterId: string): Promise<{ companyName: string; bio: string; profilePicture: string }>
}
